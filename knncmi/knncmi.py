...
def getPairwiseDistArray(data, coords=[], discrete_dist=1, periodic_vars={}):
    ...
    for coord in coords:
        ...
        if pd.api.types.is_numeric_dtype(thisdtype):
            diff = abs(data[col_names[coord]].to_numpy() - data[col_names[coord]].to_numpy()[:, None])
...
def main():
    ...
    print("PERIODIC:")
    print("MI(sin, cos):", cmi(["x"], ["y"], [], 4, df_periodic, periodic_vars=periodic_vars))  # MI(x,y)
    print("CMI(sin, cos | theta):", cmi(["x"], ["y"], ["theta"], 4, df_periodic, periodic_vars=periodic_vars))  # CMI(x,y|theta)
    print("CMI(sin, cos | noise):", cmi(["x"], ["y"], ["z"], 4, df_periodic, periodic_vars=periodic_vars))  # CMI(x,y|z)
